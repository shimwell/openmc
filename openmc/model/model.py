...
class Model:
    ...
    def convert_to_multigroup(
        ...
        method : {"material_wise", "stochastic_slab", "infinite_medium", \
                  "transport_free"}, optional
            Method to generate the MGXS. "transport_free" collapses each
            material's pointwise data deterministically (no Monte Carlo, no
            transport solve) against a narrow-resonance weighting flux with
            resonance self-shielding, and builds a deterministic P0 scattering
            matrix; it introduces no Monte Carlo noise and is valid in every
            group. See :func:`openmc.mgxs.transport_free.collapse_material`.
            ...
            if not Path(mgxs_path).is_file() or overwrite_mgxs_library:
                if method == "infinite_medium":
                    ...
                    self._generate_stochastic_slab_mgxs(
                        groups, nparticles, mgxs_path, correction, tmpdir, source_energy,
                        temperatures, temperature_settings)
                elif method == "transport_free":
                    self._generate_transport_free_mgxs(
    ...
    def _generate_transport_free_mgxs(
        ...
        Each material's pointwise cross sections are collapsed against a narrow-resonance
        (1/E + source) weighting flux with always-on resonance self-shielding (resolved
        range by 1/(sigma_t + sigma_0); unresolved range via NJOY probability tables),
        together with a deterministic P0 group-to-group scattering matrix (including a
        free-gas thermal kernel for light nuclides). Unlike "material_wise" and
        "stochastic_slab", this introduces no Monte Carlo noise and yields a positive
        total cross section in every group. See :mod:`openmc.mgxs.transport_free`.
        ...
        """
        from openmc.mgxs.transport_free import collapse_material, scatter_matrix
