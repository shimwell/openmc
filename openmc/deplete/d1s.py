...
def apply_time_correction(
    ...
    index : int or iterable of int, optional
        ...
        computed in a single pass: the tally arrays are read and reshaped once
        and shared across all indices instead of being re-read and re-reshaped
        on every call. Each derived tally is identical to the corresponding
        single-index call.
    sum_nuclides : bool
    ...
    results = []
    if sum_nuclides:
        # Apply the TCF and sum over the parent-nuclide axis for each requested
        # index. The reshaped views above are shared across all indices so the
        # tally data is only read and reshaped once. The per-index math is the
        # same broadcast-multiply-and-reduce as a scalar call, so each result is
        # bit-for-bit identical to the corresponding single-index call. sum and
        # sum_sq are left unset: the public accessors return None for any
        # derived tally (matching the scalar behavior), and storing the
        # per-nuclide arrays (shaped inconsistently with the popped filter)
        # would only waste two full-size multiplies per index and break
        # operations such as Tally.sparse.
        for t in range(len(indices)):
            tcf_b = tcf[t].reshape(1, -1, 1, 1, 1)

            new_tally = copy(tally)
            new_tally._filters = copy(tally._filters)
            new_tally._mean = (tally_mean_5d * tcf_b).sum(axis=1).reshape(flat_shape)
            new_tally._std_dev = np.linalg.norm(
                tally_std_dev_5d * tcf_b, axis=1).reshape(flat_shape)
            new_tally._sum = None
